...
from [litellm](https://docs.litellm.ai/docs/tutorials/claude_code_beta_headers)
...
### Testing
Tested with:
- â Anthropic: Header passed through correctly
- â Azure AI: Header passed through correctly  
- â Bedrock Converse: Header filtered out (returns error without fix)
...
sequenceDiagram
    ...
    Config-->>LP: Returns mapping (headerâvalue or null)
...
3. **Header transformation**: Headers are mapped to provider-specific names (e.g., `advanced-tool-use-2025-11-20` â `tool-search-tool-2025-10-19` for Bedrock)
...
- â `computer-use-2025-01-24` â `computer-use-2025-01-24` (supported, passed through)
- â `advanced-tool-use-2025-11-20` â filtered out (null value in config)
- â `unknown-header` â filtered out (not in config)
...
- Some headers are transformed (e.g., `advanced-tool-use` â `tool-search-tool`)
...
## How this proxy implements it

This proxy implements the per-provider mapping model via the transform engine
rather than a JSON config file. Operators edit `proxy_config.toml` directly —
no separate `anthropic_beta_headers_config.json` and no restart is needed for
config edits that go through the config reload path.

Two pieces work together:

1. A new Tier-2 builtin **`filter_anthropic_beta`**, attached at
   `before_upstream`. It reads the `anthropic-beta` request header
   (comma-separated, the form real Claude Code sends), filters each entry
   through the owning set's `anthropic_beta_map`, and writes the filtered
   result back.
2. A new top-level field **`anthropic_beta_map`** on `[transforms.<name>]`,
   keyed by input header name. Values are the upstream header name to emit, or
   `""` (empty string — TOML has no null scalar) to drop.
...
Filtering rules mirror LiteLLM's: entries not in the map are dropped; entries
mapped to `""` are dropped; others are emitted under their mapped name. When no
entries survive, the header is removed entirely. Scope today is
`anthropic-messages` upstreams only — the base `anthropic-beta` header is
populated by `routing.ts` for those upstreams, and the builtin is a second-stage
filter on top of that base.

See [docs/transforms-reference.md](./transforms-reference.md) §
`filter_anthropic_beta` for the full reference.
