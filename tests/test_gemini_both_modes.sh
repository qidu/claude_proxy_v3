...
EOF
  ...
  RESP=$(timeout 10 curl -s -N "$BASE/v1/messages" -H "Content-Type: application/json" \
    -d "{\"model\":\"$MODEL\",\"messages\":[{\"role\":\"user\",\"content\":\"Count 1 to 5\"}],\"max_tokens\":100,\"stream\":true}" 2>/dev/null | head -1)
  ...
  RESP=$(timeout 10 curl -s -N "$BASE/v1/interactions" -H "Content-Type: application/json" \
    -d "{\"model\":\"$MODEL\",\"input\":{\"messages\":[{\"role\":\"user\",\"content\":\"Count 1 to 5\"}]},\"stream\":true}" 2>/dev/null | head -1)
  ...
  RESP=$(timeout 10 curl -s -N "$BASE/v1beta/models/$MODEL:streamGenerateContent" -H "Content-Type: application/json" \
    -d '{"contents":[{"role":"user","parts":[{"text":"Count 1 to 5"}]}]}' 2>/dev/null | head -1)
  if echo "$RESP" | grep -q "data:"; then
    echo "✅ SSE"
    ((PASS++))
  else
...
test_mode "OpenAI-Compatible" '[models.gemini-2-5-flash]
mode = "openai-completions"
# Uses default upstream'

# Test Native Mode
test_mode "Native Gemini" '[models.gemini-2-5-flash]
mode = "native"
