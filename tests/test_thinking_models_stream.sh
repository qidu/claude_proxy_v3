...
PROXY_CONFIG_PATH=./proxy_config.toml node dist/server.js > /tmp/proxy_thinking_stream.log 2>&1 &
SERVER_PID=$!
sleep 3
...
test_endpoint() {
  ...
  if [ "$is_stream" = "true" ]; then
    # Test streaming
    RESP=$(timeout 3 curl -s -N "$url" -H "Content-Type: application/json" -d "$data" 2>/dev/null | head -1)
...
for MODEL in "${MODELS[@]}"; do
  ...
  test_endpoint "  /v1/messages" \
    "$BASE/v1/messages" \
    "{\"model\":\"$MODEL\",\"messages\":[{\"role\":\"user\",\"content\":\"2+2?\"}],\"max_tokens\":100,\"stream\":true}" \
    "true"
  
  test_endpoint "  /v1/interactions" \
    "$BASE/v1/interactions" \
    "{\"model\":\"$MODEL\",\"input\":{\"messages\":[{\"role\":\"user\",\"content\":\"3+3?\"}]},\"stream\":true}" \
    "true"
  
  test_endpoint "  streamGenerateContent" \
